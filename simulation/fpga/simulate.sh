...
ghdl -a ${OPTS} rpll.vhd
ghdl -a ${OPTS} ../../tang20k/src/i2s_simple.vhd
ghdl -a ${OPTS} ../../tang20k/src/sn76489.vhd
ghdl -a ${OPTS} ../../tang20k/src/sample_rate_converter_top.vhd

ghdl -a ${OPTS} sample_rate_converter_tb.vhd
ghdl -e ${OPTS} sample_rate_converter_tb
ghdl -r sample_rate_converter_tb --vcd="audio.vcd" --stop-time=10ms | tee audio.log
grep report audio.log | cut -d\) -f2- | cut -c2- | awk '{print $1}' | nl > audio.plot
if [ -z "$DISPLAY" ]; then
